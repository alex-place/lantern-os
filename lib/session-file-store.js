...
class FileSessionStore extends session.Store {
  ...
  // Extend-only, never create: a request in flight during logout still holds the loaded
  // session and would otherwise re-write the file destroy() just unlinked, resurrecting
  // the session (logout in one tab undone by a page load in another).
  touch(sid, sess, cb) {
    const last = this._lastSet.get(String(sid)) || 0;
    if (Date.now() - last < TOUCH_THROTTLE_MS) return cb && cb();
    fs.access(this._file(sid), (missing) => {
      if (missing) return cb && cb(); // destroyed/expired — do not resurrect
