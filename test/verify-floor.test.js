...
//
// Run: node test/verify-floor.test.js
...
fs.writeFileSync(path.join(root, "test", "alpaca-adapter.test.js"), "process.exit(0);\n");
...
check("js file → node --check + its real standalone unit test", () => {
  const cmds = deriveVerifyFloor(root, ["lib/alpaca-adapter.js"]);
  assert.deepStrictEqual(cmds, [
    "node --check lib/alpaca-adapter.js",
    "node test/alpaca-adapter.test.js",
  ]);
});
