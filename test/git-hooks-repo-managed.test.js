...
//
// Run: node test/git-hooks-repo-managed.test.js
...
async function logicChecks() {
  ...
  check("a new surface with NO loop stage is a violation", () => {
    const r = mod.evaluateSurfaces([{ path: "public/x.html", content: "<html><head><title>x</title></head></html>" }]);
    assert.strictEqual(r.ok, false);
    assert.strictEqual(r.violations.length, 1);
  });

  check("a meta-tag loop stage justifies a surface", () => {
    const r = mod.evaluateSurfaces([{ path: "public/x.html", content: '<meta name="loop-stage" content="observe">' }]);
    assert.ok(r.ok);
    assert.strictEqual(r.justified[0].stage, "observe");
  });

  check("a comment-form loop stage also justifies (reset-password used verify)", () => {
    const r = mod.evaluateSurfaces([{ path: "public/x.html", content: "<!-- loop-stage: verify -->" }]);
    assert.ok(r.ok);
