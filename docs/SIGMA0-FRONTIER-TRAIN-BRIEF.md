# Î£â‚€ Frontier Training Brief â€” the honesty-native pretrained model

**Date:** 2026-07-06 Â· **Status:** Living brief Â· **Companion:** [SIGMA0-MODEL-DESIGN-BRIEF.md](SIGMA0-MODEL-DESIGN-BRIEF.md) (the â‰¤8GB *serving* layer)
**Governance:** the program this brief designs requires an accepted ADR
([ADR-0024](adr/0024-sigma0-frontier-training-program.md), Status: Proposed â€” operator approval only).

**The two-layer contract:** TRAIN frontier â†’ SERVE everywhere. The serving brief's constraints
(local â‰¤8GB, 4-bit, format parity) are this brief's **distillation target**, not its ceiling.
Budget is a **decision input** below, never an assumed constraint.

**Frontier, defined honestly:** frontier *on the honesty axis* â€” the first model whose
pretraining objective, not its post-training patch, is calibrated honesty â€” while remaining
...
```
# DESIGN BRIEF â€” the Î£â‚€ frontier training program

## Your role
You are a frontier ML architect + training-program lead. Design the best buildable
honesty-native PRETRAINING program. Output concrete, falsifiable decisions with kill-gates â€”
not an essay. Follow the Î£â‚€ discipline (last section) in your OWN answer.
...
[MEASURED] Kalai/Nachum/Vempala/Zhang (arXiv:2509.04664): hallucination persists because
  ...
  "honesty-native pretraining" slot is unoccupied (7 verified searches, 2026-07-06 â€”
...
[MEASURED] The property is trainable and cheap at small scale: our QLoRA honesty-tune of
  Ouro-1.4B hits golden 0.958 / confab 10% / over-abstain 2.2% on 66 held-out â€” tying
...
## Ground truth â€” architecture (do not re-derive)
[MEASURED] Ouro (arXiv:2510.25741): LoopLM family, weight-tied recurrent depth, 7.7T tokens â€”
...
[MEASURED] Learned halting is the weak leg, twice over: MoEUT's SUT ablation â€” ACT was the
  main cause of SUT's poor performance; our trilogy â€” Ouro's Q-exit exits flat (~3.4/4,
...
[MEASURED] Certificate quantities are measurable on a real loop: Ouro rho_obs ~= 0.88,
  ...
  landed (certificate Â§3.1, PR #2157): commitment cost grows at the certified rate; grounding
...
[MEASURED] Barrier certificates are now trainable-with-proof: set-based training where
  loss = 0 formally proves validity (arXiv:2605.02526) â€” candidate for training-time
  stability instrumentation.
[MEASURED] Gate signals: free logprob (FLARE, arXiv:2305.06983) is the only signal with a
  positive ROUTING edge on our harness; canary/council-Î”/self-consistency out-rank but
  under-route (#2047, #2059). Design serving-time grounding around logprob.
[HEURISTIC] Certificate Â§7.2: the open risk is a TRAINED GAMER â€” honest under audit only.
...
- Two-layer contract: the model must DISTILL to the serving brief's â‰¤8GB local artifact.
...
- Honesty must be a CHECKABLE PROPERTY bound to external verification the model does not
  control â€” never marker-emission. (Â§7.2 is a first-class design input, not an appendix.)
...
## Decisions to make (each: options â†’ tradeoffs â†’ recommendation â†’ cheapest FALSIFIER
   â†’ kill-gate that would stop the program at that phase)
D1. Scale & budget tiers. Params x tokens x cluster, in GPU-hours. Anchors [MEASURED]:
    ...
    published â€” say so). Define tiers: PILOT (replicate MoEUT-scale + honesty objective),
...
D4. Objective stack & staging. Where does honesty enter: (a) pretraining data/objective
    from token one, (b) mid-training stage, (c) post-training only (the incumbent default â€”
    ...
    correlates with correctness â€” SPECULATIVE, ours; give its falsifier), STARS regularizer.
    Anti-gaming: how each term is bound to an external check (Â§7.2).
...
D6. Training-time certification. What runs DURING training: rho/conditioning tracking,
    ...
    dwell-time monitoring (if MoE). Define the collapse/instability ABORT criteria â€” the
...
D7. Eval & release gates. North star = confabulation-rate on negatives (never raw accuracy
    ...
    HaluEval, TruthfulQA. Distillation gate: the â‰¤8GB artifact must retain the honesty
    property (define retention threshold). Pre-release: measure the watched-vs-unwatched
    gap explicitly.
D8. Red-team at scale. Name the strongest Â§7.2 attack against YOUR design (trained gamer,
...
## The Î£â‚€ discipline you must follow in your answer
...
- Red-team your own design: top 3 failure modes, including at least one Â§7.2 gaming mode.
...
kill-gate each). 3. The phased program (PILOT â†’ BASE â†’ FRONTIER) with measurable gates and
...
AbstentionBench + SimpleQA-Verified + HaluEval). 7. Top 3 risks + the Â§7.2 attack you fear
most. 8. The distillation plan down to the serving brief's â‰¤8GB artifact.
```
