...
|---|---|---|---|---|---|
| **HumanEval** | Act / Reason | Code pass@1 (164 problems) | `scripts/eval_humaneval_chat.py`, `scripts/eval_humaneval_ouro.py`, `experiments/humaneval_runner.py` | ✅ Run | **Served Qwen2.5-Coder (the local coding engine, #2171): full-164 pass@1 0.72 via the chat product** (`qwen25coder-served-he164`, 2026-07-06, ollama, exec-graded) — the first graded number for the model we actually ship (#2190). Raw `ollama-direct` scored 0.95 on the first-20 vs 0.85 through the chat on the same 20 → the chat wrapper leaks ~capability (model omits `import` lines in the fenced reply; a harness/prompt artifact worth fixing, #2194). Ouro reference (kernel/research lane): full-164 **0.427** (`ouro-peak-full-ck600`) — `leaderboard.jsonl` |
