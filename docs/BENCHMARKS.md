...
**Which model are we actually running, and where** (reconciled 2026-07-06, #2188): "the local model" is a **split**, so every row below names *which model + adapter* it measures. (1) **Local coding engine → Qwen2.5-Coder-7B**, served **raw (no LoRA adapter)** via Ollama `:11434`, VRAM-gated selection in [`apps/lantern-garage/lib/local-model-registry.js`](../apps/lantern-garage/lib/local-model-registry.js) + [`data/models/local-registry.json`](../data/models/local-registry.json) (rank 1, verified). (2) **Kernel / research base → Ouro-1.4B** (`keystone-ft`) — Convergence-Core kernel, *not* a coding candidate; the loop-value experiment (#2178) came back **NEGATIVE** (recurrent depth adds no accuracy, ~15× slower), so Ouro stays research-only. (3) **LoRA adapters live on Ouro, not Qwen** — the honesty/format QLoRA tune and the loop-value 'tuned adapter' are adapter-over-frozen-Ouro; Qwen is used raw. (4) **PLT coder** (`keystone-sigma0-plt`, ~7.6B) is an owned research coder on a `:11435` shim, sorted **behind** Qwen until its on-box HumanEval head-to-head flips `verified:true`. (5) **Chat coding turns already route to Qwen** — `selectBest("coding")` in the registry returns `qwen2.5-coder:latest` (rank 1), and the #2188 full-164 run verified 164/164 turns served by Qwen. The one place still Ouro-first is the `KEYSTONE_SERVE_OURO=1` quickboot flag, and note the trap: `ouro_serve.py` binds the **same `:11434`** Ollama uses, so enabling it would *shadow* Qwen (coding routes to `qwen2.5-coder:latest@:11434`, but that port would be held by the Ouro server, which only serves Ouro). **Recommendation:** to serve coding turns with Qwen, let **Ollama own `:11434`** (it serves both the `qwen2.5-coder` and `ouro` tags) rather than running `ouro_serve` on the same port — i.e. leave `KEYSTONE_SERVE_OURO` unset, or move the Ouro research server to a distinct port.

---

**Last review: 2026-07-05** (`/benchmarks`, issue #2135 follow-up). Competitor SOTA web-validated live: SWE-bench Verified — Claude Mythos 5 **95.5%** > Claude Fable 5 95% > Claude Opus 4.8 88.6% > DeepSeek-V4-Pro-Max 80.6% (llm-stats, Jul 4); MBPP — o4-mini **94.9%** (codesota, Dec 2025); HumanEval saturated (frontier >93%, retired as differentiator). **Serving-provenance finding:** on this box the only servable checkpoints are base `Ouro-1.4B` / `-Thinking` with **no coding LoRA on disk** — both emit near-empty completions, so a fresh coding-golden-25 run scored **0% pass@1** (`leaderboard.jsonl` label `ouro-thinking-base-golden25`, ts 1783296694). The `ck600` adapter that produced 0.427 is **not present here** — but the **owned `keystone-sigma0-plt` (~7.6B LoopCoder) full weights _are_ on disk** and serve fine in 4-bit on `:11435`. Its first measured eval landed the same session: **88% pass@1 (22/25) on coding-golden** (`leaderboard.jsonl` label `keystone-plt-golden25-4bit`, reproduced twice), ~150 s/problem at 4-bit on a 12 GB box. So the servable local coder is the PLT model, not Ouro; raw-capability is a real 88% on the internal set, with the external HumanEval-164 head-to-head still owed (≈13 h locally → cloud L4/A100). SWE-bench remains the single empty differentiating cell (needs Docker, unavailable locally). **⚠️ Superseded 2026-07-06 (#2171/#2188):** the default local coder is now **Qwen2.5-Coder-7B** (Apache-2.0, Ollama `:11434`), which verified on-box at coding-golden pass@1 **0.96** (#2173, row `qwen25coder-onbox-2173`); the PLT model is retained as a research coder behind it. See the model-map note above.
...
|---|---|---|---|---|---|
| **HumanEval** | Act / Reason | Code pass@1 (164 problems) | `scripts/eval_humaneval_chat.py`, `scripts/eval_humaneval_ouro.py`, `experiments/humaneval_runner.py` | ✅ Run | **Local coding engine = Qwen2.5-Coder** (#2171, served **raw/no-adapter**): HumanEval-164 **chat-product** pass@1 **0.72** (118/164, all 164 served by `qwen2.5-coder:latest`, 3.7 s/problem, #2188 row `qwen25coder-humaneval164-2188`), coding-golden exec pass@1 **0.96** (24/25) on-box (#2173, row `qwen25coder-onbox-2173`). **Ouro-1.4B = kernel/research + adapter host**: the honesty/format QLoRA *adapter* scores pass@1 **0.90** (HumanEval n=10, depth 1; loop-value NEGATIVE, #2178), frozen-base full-164 **0.427** — `leaderboard.jsonl` |
...
| **SWE-bench Lite** | Act / Verify | Real-repo issue → patch, resolved% (official Docker/Modal grader) | `scripts/eval_swebench_chat.py`, `scripts/swe_agent_loop.py`, `scripts/swe_agentic_run.py` | 🟡 Partial | Qwen2.5-Coder **single-shot** 0/3 (2 applied-but-wrong) — but single-shot is no longer the local path: the agentic propose→test→retry loop + the #2174 verification-decides gate now front it (measured resolved% still owed; relates to #2187) |
...
- **Public SOTA (web-validated 2026-07-05, [llm-stats](https://llm-stats.com/benchmarks/humaneval)):** Claude Sonnet 4.5 **97.6%**, DeepSeek R1 **97.4%**; field average across 54 models **89.2%**. **Saturated** — every frontier model clears 90% and the set is in public training mixes, so read the **0.427** as an *absolute-capability* number for **frozen Ouro-1.4B** (kernel/research base), **not** the local coding engine. The local coding engine is now **Qwen2.5-Coder** (#2171), whose measured on-box numbers are HumanEval-164 chat-product pass@1 **0.72** (#2188) and coding-golden pass@1 **0.96** (#2173). Use SWE-bench / LiveCodeBench to differentiate the frontier.
...
- **Qwen2.5-Coder full-164 chat run (#2188, 2026-07-06):** pass@1 **0.72** (118/164), all 164 turns served locally by `qwen2.5-coder:latest` (verified via the `served_models` histogram — no silent cloud fallback), 3.7 s/problem. Ledger row `qwen25coder-humaneval164-2188` (`git_sha 12d96ed2`, campaign `qwen25coder-humaneval164-2188-fixed`). This confirms provider routing sends coding turns to Qwen and the product path delivers a real ~0.72, not a raw-model-only number.
- **Harness-correctness fix landed in the same run:** the first pass measured **0.47** because `make_candidate` (shared extractor) dropped model-emitted `import`/`from` lines when the model re-emitted the whole function inside a fence and the HumanEval prompt shipped no import (e.g. `get_positive` → `from typing import List`, `is_prime` → `from math import sqrt`) — 65 spurious `NameError`, of which 44 were pure dropped-import. The fix preserves any imports the model emits before the def (deduped against the prompt preamble); pass@1 corrected 0.47 → **0.72**. The residual failures are genuine (wrong output, or the model reaching for unavailable libs like `numpy`).
