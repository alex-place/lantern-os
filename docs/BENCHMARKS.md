...
|---|---|---|---|---|---|
| **HumanEval** | Act / Reason | Code pass@1 (164 problems) | `scripts/eval_humaneval_chat.py`, `scripts/eval_humaneval_ouro.py`, `experiments/humaneval_runner.py` | ✅ Run | **Ouro** full-164 pass@1 **0.427** (`ouro-peak-full-ck600`); newest graded coding row = Ouro ck600 **0.80** on golden-25 (`ck600-restore-check`, 2026-07-06). ⚠️ **The local coding engine is now Qwen2.5-Coder** (#2171) but it has **no graded HumanEval row** — `data/eval/humaneval/qwen-direct-he-*.jsonl` is *ungraded generations* and the coding-backend A/B report is empty; the older **0.75** subset row is **Qwen2.5-Coder-3B**+Σ₀ data (a ceiling probe). Grading the *served* Qwen is the top open gap (#2188 reconcile, #2187 SWE-bench) — `leaderboard.jsonl` |
