...
| **Retrieval A/B (internal)** | Remember | Recall@{5,10} / MRR@{3,10} of OUR production retrieval arms (lexical-idf, BM25-okapi, nomic-dense, hybrid-α, +cross-encoder) on a known-item set, split by lexical-overlap band — the A/B that gates the reranker decision (#2355) in evidence, not borrowed numbers | `scripts/eval_retrieval_ab.js` | 🟡 Partial | Bundled prose fixture (18 docs / 15 queries incl. trading narrations): lexical-idf R@5 **0.867** overall, **0.333 on the low-overlap band** vs **1.000** verbatim — the band where dense/rerank must earn their keep. Dense/hybrid/rerank arms skip-logged, pending an Ollama-equipped run + #2355 endpoint — `data/eval/retrieval/runs.jsonl` (#2356) |
