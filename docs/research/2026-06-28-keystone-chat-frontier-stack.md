...
Keystone runs the same harness everywhere and lets the **VRAM gate + serving-liveness probe (both already in [`local-model-registry.js`](../../lib/local-model-registry.js))** pick the model:
