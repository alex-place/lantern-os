...
($46$ passing in \texttt{test\_cio\_sde.py}) for the symmetric case, with $14$
...
graph: \emph{standard; a nonlinear ROA first cut is validated (\#1991)}. \S6
...
\begin{caveatbox}
\textbf{Update (2026-07-04) --- landed since this \LaTeX{} was last synced.} The
discrete-time dichotomy (\#1988), the defective-$A$ ordered-Schur split for the continuous
dichotomy (\#1989), the trigger calibration (\#1990: precision~$1.0$ / snapshot-recall~$0.08$),
and a validated region-of-attraction first cut (\#1991: $\{V\le2.31\}$ with $100\%$
inside-convergence on the reversed--Van-der-Pol benchmark) are detailed in the Markdown
certificate (the living source of record) and in \texttt{experiments/sigma0\_*.py}. This
\LaTeX{} formalization covers \S1--\S7 as summarized in each status line above; the Markdown
carries the per-date maintenance log.
\end{caveatbox}
...
\item \texttt{src/cio\_sde/collapse.py} --- \texttt{SemanticCollapseOperator},
  ...
  \texttt{ReconstructionOperator}, \texttt{stability\_gates} (\#768),
  \texttt{dichotomy\_certificate} (\#768, ordered-Schur split incl.\ defective $A$, \#1989),
  and \texttt{discrete\_dichotomy\_certificate} (\#1988).
\item \texttt{src/cio\_sde/surprise.py} --- \texttt{SurpriseMonitor} and the Kalman
  measurement update (the missing inference step).
\item \texttt{tests/test\_cio\_sde.py} --- 46 passing; Theorem~1
