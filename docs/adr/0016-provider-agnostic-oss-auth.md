...
- **Follow-ups:**
  - Password reset + email verification once email infra exists.
  - Remove the `req.session.patreon` compat shim one release after rollout.
  - Discord guild-role → Keystone-role mapping (currently Discord/Google/local default to the free
    tier; Patreon remains the paid-tier source of truth).
