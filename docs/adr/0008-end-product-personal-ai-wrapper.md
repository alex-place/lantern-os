...
([`tool-runner.js:33-136`](../../lib/tool-runner.js)). The trained Ouro adapter
...
2. **Capabilities are Tools + Skills, not subsystems.** Every new user-facing ability is added as a
   **Tool** (a `name + input + output + success` capability — the core Tool object,
   ...
   ([`tool-runner.js`](../../lib/tool-runner.js) /
...
3. **Model-agnostic capability.** Tools/Skills are independent of which model calls them (ADR-0005):
   ...
   ([`tool-runner.js:150` preamble](../../lib/tool-runner.js);
   [`stream-chat.js:1579-1634` local tool loop](../../lib/stream-chat.js)).
...
| Only model-callable tools today are 7 repo-coding tools, repo-sandboxed | [`tool-runner.js:33-136`](../../lib/tool-runner.js) | High | code |
| Local model gets a tool-exec loop when `CHAT_TOOL_EXEC=1` | [`stream-chat.js:1579-1634`](../../lib/stream-chat.js) | High | code |
