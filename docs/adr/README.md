...
| [0034](0034-moe-core-admission-switched-system-gate.md) | MoE for the Σ₀ core — **adopted as soon as feasible** (operator, 2026-07-24); *feasible* = the switched-system admission gate exists and passes: per-expert contraction receipts (JSRR per active expert), **dwell-time certification** with a serve-time dwell monitor, router-entropy/churn canaries, external verification unchanged. Until then the dense ≤3B core remains the product and Part-I stability numbers stay forbidden for routed loops (cert §1.2.2). Gate tooling moves to the FRONT of the R lane; envelope binds on **footprint** (≤4GB active/memory), not total sparse params | Accepted (Alex Place, in-session, 2026-07-24) |
