...
| [0031](0031-spiral-coder-arc-agi2-efficiency-target.md) | Spiral Coder — names the external niche for ADR-0030: the ARC-AGI-2 **budgeted-efficiency** track (score vs $/task) via execution-guided **inductive** program synthesis; output=program, halt=exact verifier (not learned ACT); adds a **held-out generalization gate** (a real exec-verified run showed a memorizer passes all train tests yet fails held-out); SOTA bar = beat equal-compute Best-of-N + land on the budgeted Pareto frontier at ~$0 API | Accepted (Alex Place, in-session, 2026-07-23) |
| [0032](0032-real-money-trading-onboarding.md) | Real-money onboarding — the contract for how a user crosses paper→live safely: live keys are a **separate explicit credential** (never a promotion of paper keys), enabling live is a **multi-step opt-in** behind the operator double-gate + a risk attestation, **manual** live orders need per-order confirmation (never one-click), **autopilot** on real money stays gated on the ADR-0028 Sharpe-CI mandate, and the assistant never places trades. Design only — **no live trading is enabled** | Proposed (awaiting Alex's approval) |
| [0033](0033-apex-max-acceptable-risk-trader.md) | Apex trader — maximize profit at the highest **acceptable** risk: acceptability defined by the un-stoppable overnight-gap tail (stress bound: a −6% gap costs ≤ ~20% equity ⇒ **hard 3× cap**, 5×/Kelly rejected), composing the measured edges — SPY overnight vol-gated 3× core + QQQ calm-night complement + T-bill carry + a **measurement-gated** convex options sleeve — with brake scaling + circuit breakers. Paper-first; real money stays behind ADR-0028/0032. Design only | Proposed (awaiting Alex's approval) |
| [0034](0034-moe-core-admission-switched-system-gate.md) | MoE for the Σ₀ core — **adopted as soon as feasible** (operator, 2026-07-24); *feasible* = the switched-system admission gate exists and passes: per-expert contraction receipts (JSRR per active expert), **dwell-time certification** with a serve-time dwell monitor, router-entropy/churn canaries, external verification unchanged. Until then the dense ≤3B core remains the product and Part-I stability numbers stay forbidden for routed loops (cert §1.2.2). Gate tooling moves to the FRONT of the R lane; envelope binds on **footprint** (≤4GB active/memory), not total sparse params | Proposed (recording the in-session operator decision; awaiting Alex's formal approval) |
