...
> **Nav is partly runtime-injected.** `js/site-chrome.js` builds the top nav and footer from its `NAV_LINKS` array (plus `FOOTER_EXTRA_LINKS`, footer-only surfaces such as Create and Explore), so those links never appear in a page's static markup. `build-nav-map.mjs` reads both arrays and attributes them to every page that loads the script — without that, the map would badly understate reachability.
>
> **The footer is global.** As of 2026-08-01 the four hand-rolled `<footer class="site-footer">` copies (`index`, `kalshi-terminal`, `orchestration`, plus the chrome-less `auth`) are gone; `site-chrome.js` owns the markup, the status dot, and the edition · version · commit line. `auth.html` keeps a local footer because it deliberately does not load the app chrome (a logged-out page should not render the app nav).
