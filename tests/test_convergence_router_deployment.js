 ...
 *
 * Tests the convergence router's integration with the Keystone runtime for:
 * - Intent routing (6 agents: lantern, blinkbug, keystone, waterfall, xenon, founder)
 * - Cache validation and staleness detection
 * - Pattern persistence
 * - Token efficiency measurement (90% local, 10% external)
 * - Hit rate tracking (>70% target)
 *
 * No live server required — tests the convergence router directly.
 */
...
async function run() {
  ...
  test("ConvergenceRouter constructs successfully", () => {
    ...
    assert.ok(router.convergenceModel, "convergence model should be loaded");
  });

  test("getRouter returns singleton instance", () => {
    const r1 = getRouter();
    const r2 = getRouter();
    assert.strictEqual(r1, r2, "getRouter should return the same instance across calls");
  });
  ...
  await testAsync("routeIntent returns one of 6 valid agents", async () => {
    ...
    assert.ok(validAgents.includes(result.agent), `agent ${result.agent} should be one of 6 personas`);
  });

  await testAsync("routeIntent includes confidence score (0-100)", async () => {
    const router = new ConvergenceRouter();
  ...
  await testAsync("cache_validated returned for high-confidence cached match", async () => {
    const router = new ConvergenceRouter();
    const uniqueMsg = "unique debug routing message " + Date.now();
    const first = await router.routeIntent(uniqueMsg);
    assert.ok(["keystone_routing", "cache_validated"].includes(first.source), "first call should route");
  ...
  test("pattern cache structure is valid", () => {
    ...
    assert.ok(typeof patterns.marketPatterns === "object", "should have marketPatterns object");
    assert.ok(typeof patterns.intentPatterns === "object", "should have intentPatterns object");
  });
  ...
  await testAsync("deterministic tasks map to local endpoints", async () => {
    ...
    assert.strictEqual(result.source, "deterministic_route", "should route deterministically");
  ...
  await testAsync("90% of deterministic tasks route locally", async () => {
    ...
    assert.ok(localPercentage >= 90, `should route 90% locally, got ${localPercentage.toFixed(1)}%`);
  });
  ...
  test("getStats returns all required metrics", () => {
    ...
    assert.strictEqual(typeof stats.cachedMarketPatterns, "number", "should have market pattern count");
    assert.strictEqual(typeof stats.cachedIntentPatterns, "number", "should have intent pattern count");
    assert.strictEqual(typeof stats.cachedCodePatterns, "number", "should have code pattern count");
  });
