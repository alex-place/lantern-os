...
server.listen(0, "127.0.0.1", async () => {
  ...
  try {
    ...
    // the confirmation link carries).
    const { createToken } = require(path.join(LIB, "lib/auth-tokens"));
    const { getProfileByEmail } = require(path.join(LIB, "lib/user-profiles"));
    const prof = getProfileByEmail("dev@example.com");
    const vtok = createToken("verify_email", prof.id, null);
    r = await c("GET", "/api/auth/verify-email?token=" + encodeURIComponent(vtok));
    ok("verify-email redirects with success", r.status === 302 && /verify=1/.test(r.location || ""));
