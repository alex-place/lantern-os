...
function convergenceVerdict({ t, direction, sr, struct, candle, marketStatus, news_sentiment = 0, volume_ratio, macd_hist, ma_signal }) {
  const evInput = {
    direction,
    news_sentiment,
...
async function scanAll(watchlist) {
  ...
  for (const t of list) {
    ...
    if (gate.actionable) {
      ...
      const convergence = convergenceVerdict({ t, direction, sr, struct, candle, marketStatus, news_sentiment, volume_ratio, macd_hist, ma_signal });
