...
function sliceFor(question) {
  ...
  for (const [keys, band] of KEYMAP) {
    if (keys.some((k) => q.includes(k))) return Object.assign({ band, matched: true }, BANDS[band]);
  }
  // No cosmic-time keyword hit: report the NOW slice as the best-grounded default,
  // but flag it un-matched so callers can decline to ground a non-cosmology question.
  return Object.assign({ band: NOW, matched: false }, BANDS[NOW]);
}

// The grounding block — ONLY for questions that actually land in a cosmic-time band.
// #1275: previously this fell back to the NOW band for EVERY question, so cosmology
// grounding (dark energy / heat death / Planck epoch) was prepended to unrelated
// prompts (resume help, coding…). Return "" when there is no real keyword match.
