...
// 4. Coding capability rate — pass rates for SWE-bench and HumanEval-chat.
// Reads from specific log files generated by the evaluation scripts.
function codingCapabilityRate(sweBenchRecords, humanEvalChatRecords) {
  const sweBenchPassed = (sweBenchRecords || []).filter(r => r.passed).length;
  const sweBenchTotal = (sweBenchRecords || []).length;
  const humanEvalChatPassed = (humanEvalChatRecords || []).filter(r => r.passed).length;
  const humanEvalChatTotal = (humanEvalChatRecords || []).length;

  const sweBenchRate = pct(sweBenchPassed, sweBenchTotal);
  const humanEvalChatRate = pct(humanEvalChatPassed, humanEvalChatTotal);

  return {
    sweBench: {
      n: sweBenchTotal,
      passed: sweBenchPassed,
      rate: sweBenchRate,
      status: statusFor(sweBenchRate, sweBenchTotal),
    },
    humanEvalChat: {
      n: humanEvalChatTotal,
      passed: humanEvalChatPassed,
      rate: humanEvalChatRate,
      status: statusFor(humanEvalChatRate, humanEvalChatTotal),
    },
  };
}
...
function computeOutcomeMetrics(repoRoot) {
  ...
  // New log files for coding capability metrics
  const sweBenchLogs = readJsonl(d("eval", "swe-bench-results.jsonl"));
  const humanEvalChatLogs = readJsonl(d("eval", "human-eval-chat-results.jsonl"));
  ...
  return {
    ...
    codingCapabilityRate: codingCapabilityRate(sweBenchLogs, humanEvalChatLogs),
  };
}
...
function loadOrCaptureBaseline(metrics, repoRoot) {
  ...
  try {
    ...
    const baseline = {
      ...
      sweBenchRate: metrics.codingCapabilityRate.sweBench.rate,
      humanEvalChatRate: metrics.codingCapabilityRate.humanEvalChat.rate,
