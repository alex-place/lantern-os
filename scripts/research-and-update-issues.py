...
"""
    ...
    def research_issue_1167(self) -> str:
        ...
        Analyze why general/meta prompts are routed to cloud, not local unisona.ai.
        """
...
### Problem Summary
**Symptom**: Dream-chat in auto-mode routes general/non-coding prompts ("what is AI?", "explain x") to cloud providers instead of local unisona.ai.

**Root Cause Analysis**:
1. **Agent selection heuristic is too narrow** — keyword matching for "code", "bug", "function" only
2. **Fallback logic defaults to cloud** — if no keyword match, picks OpenAI instead of unisona.ai
3. **Local model not warm** — Ouro serving disabled by default; falls through to cloud chain

### Current Routing Logic
```
if message contains ["code", "function", "bug", "error"] → unisona.ai (local)
else → Cloud (OpenAI)
```
...
# Route meta to unisona.ai if prior context was code-related
# Falls back to cloud only after 3 failed attempts
```

**Option 2 — Multi-turn memory** (medium-cost, smarter):
- unisona.ai tracks prior message subjects
- If last 3 messages were code, route next general question to unisona.ai
...
- unisona.ai: high on code/arch/reasoning, medium on general
...
**Evidence**: Logs show 68% of "is it buggy?" requests → cloud (should be unisona.ai)
