...
module.exports = { downloadVideo, analyzeForResearch, research, storeFeatures, aggregateEditingPriors, aggregateHookPriors, aggregateHighlightPriors, aggregateFacecamPriors, rebuildCorpusDb, calibrateWeights, researchNightly, searchArchiveOrg, searchPeerTube, searchWikimedia, licenseAllowed, listDatasets, ingestDataset };
...
if (require.main === module) {
  ...
  (async () => {
    ...
    if (process.argv.includes("--list-datasets")) { emit(listDatasets()); return; }
    const dsArg = process.argv.find((a) => a.startsWith("--dataset="));
    if (dsArg) {
      const limArg = process.argv.find((a) => a.startsWith("--limit="));
      emit(await ingestDataset(dsArg.split("=")[1], { limit: limArg ? Number(limArg.split("=")[1]) : undefined }));
      return;
    }
    const src = process.argv[2];
    if (!src) { console.error("usage: open-video-research.js <url|file [--local]> | --aggregate | --calibrate | --nightly [--limit=N] | --list-datasets | --dataset=<name> [--limit=N]"); process.exit(1); }
