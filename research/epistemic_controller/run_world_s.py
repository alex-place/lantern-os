...
ARMS
  hold     HoldController fresh per episode, policy fixed. The scientist that fixes mistakes but
           never learns from them.
  self     HoldController fresh per episode, policy owned by one persistent SelfModel.
...
def run_seed(seed, arm, kind="H"):
    sm = SelfModel(cost_reference="median") if arm == "self-loose" else SelfModel() if arm == "self" else None
    eps = []
...
def main(n_seeds=100):
    ...
    H = {"hold": block(hold), "self": block(selfa), "self-loose": block(loose)}
    ...
    print(f"C1 (no proxy):      drops/ep {c1_drops:.2f}  policy changed in {100*fired(c1):.0f}% of seeds (strict) / "
          f"{100*fired(c1L):.0f}% (loose)  null-diagnoses {c1_nulls}")
    print(f"C2 (shift burst):   drops/ep {c2_drops:.2f}  policy changed in {100*fired(c2):.0f}% of seeds (strict) / "
          f"{100*fired(c2L):.0f}% (loose)  null-diagnoses {c2_nulls}")
    ...
    verdict = ("SELF EXPERIMENT STANDS" if S2 and S3 else
    ...
    out = {"n_seeds": n_seeds, "episodes": EPISODES, "H": H, "controls": {"C1_fired": fired(c1), "C2_fired": fired(c2),
           "C1_drops": c1_drops, "C2_drops": c2_drops}, "gates": gates, "verdict": verdict}
