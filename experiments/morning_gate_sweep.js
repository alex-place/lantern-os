/**
 * morning_gate_sweep.js — re-sweep TRADER_IBS_MAX_MORNING under the stack as
 * armed 2026-08-24 (operator, after Monday's missed-trade scan).
 *
 * On 8/24 six names sat at their session lows at the 10:00 decision minute —
 * SOXL 0.09, SMH 0.10, XLK 0.16, GLD 0.16, QQQ 0.22, TQQQ 0.22 — all far below
 * the regular 0.30 threshold but above the 0.08 morning gate, which blocked
 * five of the six; SOXL missed by 0.01 and waited until 11:00. The gate was
 * last validated in #3434's stack sweep, but the exit structure has changed
 * twice since (bounce-only exits, then the session-maturity gate + the 0.5%
 * stepped floor), so the knob is due for a re-sweep on the current stack.
 *
 * Bar: the gate applies to bars closing before 11:00 ET, which under the hourly
 * cadence is exactly ONE decision a day. Daily bars close at 16:00 and cannot
 * express it at all, so the surfaces are the hourly 2y halves (h1 chooses, h2
 * confirms once) plus four bar sizes on the recent window with the live 60m/:00
 * cadence modelled.
 * Usage: node experiments/morning_gate_sweep.js
 */
"use strict";
  ...
  for (const t of timeline) {
    ...
    for (const [sym, pos] of [...open]) {
      ...
      if (exitPx != null) {
        ...
        trades.push({ sym, d, entryMin: pos.entryMin, ret: exitPx / pos.entry - 1, pnlEq: pos.qtyVal * (exitPx / pos.entry - 1), reason, sessions: b.si - pos.entrySi });
    ...
    if (open.size < o.maxConc) {
      ...
      for (const sym of syms) {
        ...
        if (intraday && o.cadence && ((b.closeMin - (o.cadencePhase || 0)) % o.cadence) !== 0) continue;
        const v = intraday ? rIbs(b) : dIbs(b);
        const thr = intraday && b.closeMin < (o.morningUntil || 660) && o.morningThr != null ? o.morningThr : o.thr;
        if (v > thr) continue;
      ...
      for (const c of cands) {
        ...
        open.set(c.sym, { entry: entryPx, entry0: entryPx, qtyVal: size, qtyVal0: full, stopPx: o.stopPct == null ? null : entryPx * (1 - o.stopPct), entrySi: c.b.si, entryT: c.b.t, entryMin: c.b.closeMin, peak: entryPx });
      }
    }
...
(async () => {
  ...
  const SETS = [["1h", 60], ["30m", 30], ["15m", 15], ["5m", 5]];
  const hourly = {}, fine = {};
  for (const s of SYMS) {
    hourly[s] = annotate(await chart(s, "1h", nowSec - 720 * 86400), true);
    for (const [iv, m] of SETS) { fine[iv] = fine[iv] || {}; fine[iv][s] = annotate(await chart(s, iv, nowSec - 58 * 86400), true, m * 60 * 1000); }
  }
  const vix = priorCloseMap(annotate(await chart("^VIX", "1d", nowSec - 400 * 86400), false));
  const H_START = hourly.SPY[0].d, H_MID = hourly.SPY[Math.floor(hourly.SPY.length / 2)].d, END = "2099-01-01";
  const F_START = SETS.map(([iv]) => fine[iv].SPY[0].d).sort().slice(-1)[0];
  const W = { SOXL: 1.5, SMH: 1.5, QQQ: 1.5, IWM: 1.02, XLK: 1.0, SPY: 0.83, DIA: 0.71, GLD: 0.5, TLT: 0.5 };
  // the stack EXACTLY as armed tonight: tilt, slot order, step floor 0.5%, bounce 0.6, 3% stop, x1.5 VIX>=20
  const ARMED = { ...MONDAY, weights: W, slotOrder: "expectancy", stepFloor: 0.5 };
  const VARIANTS = [
    ["0.08 (armed)", 0.08], ["0.12", 0.12], ["0.15", 0.15], ["0.20", 0.20], ["0.25", 0.25], ["0.30 = no gate", 0.30], ["off (null)", null],
  ];
  const cellOf = (r, a, z) => { const s = score(r.curve, a, z), st = stats(r.trades, a, z); return { s, st, txt: `n ${String(st.n).padStart(4)} wr ${(st.wr * 100).toFixed(0)}% b ${st.b.toFixed(2)} ${pct(s ? s.tot : 0, 7)} ÷${ratio(s)} dd ${pct(s ? s.dd : 0, 6)}` }; };
  console.log("MORNING GATE under the stack ARMED 2026-08-24 (tilt + slot order + STEP FLOOR 0.5% + bounce 0.6 + 3% stop + hourly cadence at :00).");
  console.log("The gate applies to bars closing before 11:00 ET. With the hourly cadence that is exactly ONE decision a day (the 10:00 window).");
  console.log("Daily bars cannot express it (their close is 16:00), so the bar here is: hourly 2y halves (h1 chooses, h2 confirms) + four bar sizes on the recent window.\n");
  console.log("HOURLY 2y — entries on the :30 hourly closes (Yahoo's hourly grid), morning gate on the first bar of each session");
  const hRows = [];
  for (const [name, mt] of VARIANTS) {
    const r = simulate(hourly, SYMS, { ...ARMED, morningThr: mt }, true, vix);
    const h1 = cellOf(r, H_START, H_MID), h2 = cellOf(r, H_MID, END);
    hRows.push({ name, mt, h1, h2 });
    console.log(`  morning ${name.padEnd(15)} h1 ${h1.txt}  |  h2 ${h2.txt}`);
  }
  const base = hRows[0];
  const fitBest = hRows.slice().sort((a, z) => rat(z.h1.s) - rat(a.h1.s))[0];
  const ok = rat(fitBest.h2.s) >= rat(base.h2.s) * 0.98;
  console.log(`  → fit (h1) winner: morning ${fitBest.name}${fitBest === base ? " — the armed setting" : ` — h2 ${ok ? "CONFIRMS" : "REJECTS"} (÷${ratio(fitBest.h2.s)} vs armed ÷${ratio(base.h2.s)})`}`);
  console.log(`  → beats armed on the h2 holdout: ${hRows.filter((x) => x !== base && rat(x.h2.s) > rat(base.h2.s)).map((x) => x.name).join("; ") || "none"}`);

  console.log(`\nFINE BARS ${F_START}..today — entries only in the 60m/:00 decision windows (the live cadence), same stack`);
  for (const [name, mt] of VARIANTS) {
    const cells = SETS.map(([iv]) => { const r = simulate(fine[iv], SYMS, { ...ARMED, morningThr: mt, cadence: 60, cadencePhase: 0 }, true, vix, F_START, END); const c = cellOf(r, F_START, END); return `${iv.padStart(3)}: ${c.txt}`; });
    console.log(`  morning ${name.padEnd(15)} ${cells.join(" | ")}`);
  }
  console.log("\n  NOTE: this surface did NOT discriminate — every variant returns the identical book and ZERO pre-11:00 entries, because in the sim the\n  concurrency cap is still full of prior-session positions when the first window of the day opens (live, the overnight book had just been\n  liquidated, so a slot was free). Treat the fine-bar rows as NOT EVIDENCE either way; the verdict rests on the hourly halves above. A gate change must beat 0.08 on h2 AND not decay as bars get finer.");

  console.log("\nDIAGNOSTIC — entries at the 10:00 window, the ONE bar the gate governs (5m grid)");
  for (const [name, mt] of VARIANTS) {
    const r = simulate(fine["5m"], SYMS, { ...ARMED, morningThr: mt, cadence: 60, cadencePhase: 0 }, true, vix, F_START, END);
    const early = r.trades.filter((x) => x.entryMin != null && x.entryMin < 660);
    console.log(`  morning ${name.padEnd(15)} total ${String(r.trades.length).padStart(4)}  pre-11:00 entries ${String(early.length).padStart(3)}  mean ${early.length ? (early.reduce((a, x) => a + x.ret, 0) / early.length * 100).toFixed(3) : "  -  "}%/trade`);
  }
  console.log("\nWHAT THE GATE ACTUALLY DECIDES — first-bar entries per variant (hourly 2y, whole window)");
  for (const [name, mt] of VARIANTS) {
    const r = simulate(hourly, SYMS, { ...ARMED, morningThr: mt }, true, vix);
    const tr = r.trades.filter((x) => x.d >= H_START);
    const first = tr.filter((x) => x.entryMin != null && x.entryMin < 660);
    const st = stats(tr, H_START, END);
    console.log(`  morning ${name.padEnd(15)} total trades ${String(st.n).padStart(4)}  exp ${(st.exp * 100).toFixed(3)}%/trade  avg win ${(st.aw * 100).toFixed(2)}%  avg loss ${(st.al * 100).toFixed(2)}%`);
  }
