/**
 * loss_cut_lab.js — restore the LOSS half of the pre-round-7 exit stack without
 * re-introducing the winner clipping round 7 correctly removed (2026-08-26).
 *
 * Live evidence, stable book:
 *   period        losers  median hold   mean loss%   avg loss   payoff
 *   week 1            15       85 min       -0.35%      -$155     2.37
 *   since 08-17        9    1,481 min       -1.44%      -$835     0.65
 *
 * Week 1 cut its losers with signal_exit — 11 of 15, average -0.18%. Round 7 set
 * ZONE_EXIT / TAKE_PROFIT_R / MOMENTUM_EXIT / EXIT_MIN_PWIN to 0 (correctly: 462% vs
 * 1,494% on the 26y holdout) and armed the IBS bounce gate. What remains cannot fire
 * on a position that only falls — the bounce needs a RECOVERY to IBS >= 0.6, floor and
 * trail need a prior gain — so a trade that goes straight down has nothing between
 * entry and the -3% stop.
 *
 * The cut models exactly that gap and nothing else: past X% underwater, sell at the
 * bar close. It cannot fire above water, so the bounce still owns every winner.
 * Usage: node experiments/loss_cut_lab.js [--env C:/dev/lantern-os-stable/.env.local]
 */
"use strict";
...
function simulate(barsBySym, syms, cfg, intraday, vix, from, to) {
  ...
  for (const t of timeline) {
    ...
    for (const [sym, pos] of [...open]) {
      ...
      // LOSS-SIDE CUT: past lossCut% underwater, sell at this bar's close. It can only
      // fire BELOW water, so the bounce/floor/trail winner path is untouched — which is
      // what makes this separable from the round-7 exits that clipped winners.
      if (exitPx == null && o.lossCut > 0 && b.c <= pos.entry * (1 - o.lossCut / 100)) {
        exitPx = b.c * (1 - SLIP); reason = "loss_cut";
      }
      if (exitPx == null) {
...
(async () => {
  ...
  const envPath = argv.includes("--env") ? argv[argv.indexOf("--env") + 1] : "C:/dev/lantern-os-stable/.env.local";
  const env = {};
  if (fsx.existsSync(envPath)) for (const l of fsx.readFileSync(envPath, "utf8").split(/\r?\n/)) { const m = l.match(/^([A-Z0-9_]+)=(.*)$/); if (m) env[m[1]] = m[2]; }
  const num = (k, d) => (env[k] != null && env[k] !== "" && Number.isFinite(Number(env[k])) ? Number(env[k]) : d);
  const weights = {};
  for (const part of String(env.TRADER_SYMBOL_SIZE_MULT || "").split(",")) { const [s, w] = part.split(":"); if (s && Number(w) > 0) weights[s.trim().toUpperCase()] = Math.min(2, Math.max(0.25, Number(w))); }
  const ARMED = {
    ...MONDAY,
    thr: num("TRADER_IBS_MAX", MONDAY.thr), morningThr: num("TRADER_IBS_MAX_MORNING", MONDAY.morningThr),
    ...
    decarry: env.TRADER_EOD_DECARRY === "0" ? null : new Set(["SOXL"]),
  };
  // the live exit structure, if something still pre-empts the bounce (the lab's model of the ladder)
  if (env.TRADER_ZONE_EXIT !== "0") Object.assign(ARMED, { noBounce: true, tp: 0.03, giveback: { arm: 0.027, frac: 0.4 } });
  else if (env.TRADER_TAKE_PROFIT_R !== "0") Object.assign(ARMED, { tp: 0.03 * num("TRADER_TAKE_PROFIT_R", 1) });
  ...
  const SURF = [["h1", H_START, H_MID, "h"], ["d-fit", "2000-01-01", "2015-01-01", "d"], ["h2", H_MID, H_END, "h"], ["d-hold", "2015-01-01", "2099-01-01", "d"]];
  const run = (cfg) => {
    const h = simulate(hourly, SYMS, cfg, true, vix), d = simulate(daily, SYMS, cfg, false, vix);
    const o = {};
    for (const [k, a, z, src] of SURF) { const x = src === "h" ? h : d; const sc = score(x.curve, a, z), st = stats(x.trades, a, z);
      o[k] = { tot: sc.tot, dd: sc.dd, ratio: +(sc.dd < 0 ? sc.tot / -sc.dd : 0).toFixed(2), n: st.n, wr: st.wr, b: st.b }; }
    o._h = h; o._d = d;
    return o;
  };
  console.log("LOSS-SIDE CUT — sell a LOSER at the bar close past X% underwater. Winners untouched.\n");
  console.log(`  env: ${envPath}`);
  console.log(`  ${"lossCut".padEnd(10)}` + SURF.map(([k]) => (k + " ret").padStart(11) + "÷DD".padStart(8)).join("") + "   avg loss%  payoff");
  const rows = [];
  for (const lc of [0, 0.5, 0.75, 1.0, 1.5, 2.0]) {
    const o = run({ ...ARMED, lossCut: lc });
    const tr = o._d.trades.filter((t) => t.d >= "2015-01-01");
    const losers = tr.filter((t) => t.ret < 0), winners = tr.filter((t) => t.ret > 0);
    const avgL = losers.length ? losers.reduce((a, t) => a + t.ret, 0) / losers.length * 100 : 0;
    const avgW = winners.length ? winners.reduce((a, t) => a + t.ret, 0) / winners.length * 100 : 0;
    rows.push({ lc, o, avgL, payoff: avgL !== 0 ? Math.abs(avgW / avgL) : 0 });
    console.log(`  ${(lc === 0 ? "off" : lc.toFixed(2) + "%").padEnd(10)}`
      + SURF.map(([k]) => pct(o[k].tot, 11) + String(o[k].ratio).padStart(8)).join("")
      + `   ${avgL.toFixed(2)}%`.padStart(11) + `  ${(avgL !== 0 ? Math.abs(avgW / avgL) : 0).toFixed(2)}`.padStart(8));
  }
  const base = rows[0];
  const cand = rows.slice(1);
  const fitWin = cand.slice().sort((a, b) => (b.o.h1.ratio + b.o["d-fit"].ratio) - (a.o.h1.ratio + a.o["d-fit"].ratio))[0];
  const holdOk = fitWin.o.h2.ratio >= base.o.h2.ratio && fitWin.o["d-hold"].ratio >= base.o["d-hold"].ratio;
  console.log(`\n  fit winner: lossCut ${fitWin.lc}%  (h1 ÷${fitWin.o.h1.ratio} + d-fit ÷${fitWin.o["d-fit"].ratio} vs off ÷${base.o.h1.ratio} / ÷${base.o["d-fit"].ratio})`);
  console.log(`  holdout:    ${holdOk ? "CONFIRMS" : "REJECTS"}  (h2 ÷${fitWin.o.h2.ratio} vs ÷${base.o.h2.ratio}, d-hold ÷${fitWin.o["d-hold"].ratio} vs ÷${base.o["d-hold"].ratio})`);
  console.log(`  loss size:  ${base.avgL.toFixed(2)}% -> ${fitWin.avgL.toFixed(2)}%   payoff ${base.payoff.toFixed(2)} -> ${fitWin.payoff.toFixed(2)}`);
})().catch((e) => { console.error("loss_cut_lab failed:", e.message); process.exit(1); });
