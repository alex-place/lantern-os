/**
 * polarity_mode_lab.js — #3456 measured the inverses under the WRONG RULE.
 *
 * That lab modelled polarity as "the underlying sits at its session top"
 * (uIbs >= 1 - ibsMax). That is scan.js mode TRADER_SHORT_EDGE=1. The boxes run
 * TRADER_SHORT_EDGE=selective, which never evaluates uIbs at all — it asks two
 * different questions (#3349): has the wrapper already collapsed, and is the
 * underlying ripping. So #3456's headline ("adding 3x inverses flips down days
 * from 48%/-0.870% to 65%/+1.237%") is a claim about a rule that is not armed.
 *
 * This lab runs all four modes over the same universe and the same windows, so
 * the question becomes answerable: does the ARMED rule capture the down-day edge
 * the session-top rule showed, or does #3456's result belong to a rule we are
 * not running?
 *
 * Surfaces: hourly 2y halves (h1 fit / h2 holdout) are the honest test — both
 * `selective` inputs are % from the SESSION open, which only exists intraday.
 * The daily surface is reported because the two-window bar asks for it, with the
 * caveat that a daily "session" is one bar, so wrapperDD/underlyingTape collapse
 * to that bar's own open-to-close. Read the daily columns for the "top" and
 * "none" modes; treat the daily `selective` columns as indicative only.
 *
 * Usage: node experiments/polarity_mode_lab.js
 */
"use strict";
...
function annotate(bars, intraday) {
  let day = null, si = -1, runH = 0, runL = 0, sessO = null;
  for (let i = 0; i < bars.length; i++) {
    const b = bars[i];
    const d = intraday ? ET_DAY(b.t) : new Date(b.t).toISOString().slice(0, 10);
    if (d !== day) { day = d; si++; runH = -Infinity; runL = Infinity; sessO = b.o; }
    runH = Math.max(runH, b.h); runL = Math.min(runL, b.l);
    b.si = si; b.runH = runH; b.runL = runL; b.d = d;
    b.sessO = sessO;                       // session open — the base for both `selective` inputs
    b.ddPct = sessO > 0 ? (b.c / sessO - 1) * 100 : null;   // this instrument's % from its session open
  ...
  for (const t of timeline) {
    ...
    if (open.size < o.maxConc) {
      ...
      for (const sym of syms) {
        ...
        // POLARITY — the four modes scan.js actually implements, so the lab can be
        // asked about the one that is ARMED rather than the one that is convenient.
        //   "none"       inverses never enter (TRADER_SHORT_EDGE=0, the #3296 default)
        //   "top"        underlying at its session TOP (TRADER_SHORT_EDGE=1) — the rule
        //                #3456 modelled, and NOT the rule running live
        //   "selective"  the ARMED rule (#3349): the wrapper has not already collapsed
        //                AND the underlying is not ripping. No uIbs condition at all.
        //   "both"       selective AND the session-top thesis
        if (UNDERLYING[sym]) {
          const mode = o.polarity || "top";
          ...
          if (mode === "selective" || mode === "both") {
            // wrapperDD / underlyingTape are % from each instrument's own session open.
            // On the daily surface a "session" is one bar, so these read the bar's own
            // open-to-close — a coarse but honest analogue; the hourly surface is the
            // one this mode can really be judged on.
            const wDD = b.ddPct, uT = ub.ddPct;
            if (wDD == null || uT == null) continue;                 // scan.js: "cannot certify"
            if (wDD <= -(o.maxDD == null ? 1.5 : o.maxDD)) continue; // already collapsed
            if (uT >= (o.maxUT == null ? 0.5 : o.maxUT)) continue;   // underlying ripping
          }
        }
        const v = intraday ? rIbs(b) : dIbs(b);
...
(async () => {
  ...
  const spyD = daily.SPY, spyH = hourly.SPY;
  const H_START = spyH[0].d, H_MID = spyH[Math.floor(spyH.length / 2)].d, END = "2099-01-01";
  ...
  // SPY close-to-close direction, for the down-day question
  const dirByDay = new Map();
  for (let i = 1; i < spyD.length; i++) dirByDay.set(spyD[i].d, spyD[i].c < spyD[i - 1].c ? "down" : "up");
  ...
  const cell = (r, i) => `${pct(r.s[i].tot, 9)} ÷${String(ratio(r.s[i])).padStart(5)} wr ${(r.st[i].wr * 100).toFixed(0).padStart(2)}% n ${String(r.st[i].n).padStart(4)}`;
  const line = (n, r) => `  ${n.padEnd(30)} h1 ${cell(r, 0)} | h2 ${cell(r, 2)} | d-fit ${cell(r, 1)} | d-hold ${cell(r, 3)}`;

  const ROWS = [
    ["none — longs only", { ...ARMED, polarity: "none" }, SYMS],
    ["top — underlying at session top", { ...ARMED, polarity: "top" }, ALL],
    ["selective — THE ARMED RULE", { ...ARMED, polarity: "selective" }, ALL],
    ["both — selective AND top", { ...ARMED, polarity: "both" }, ALL],
  ];
  console.log("POLARITY MODE — #3456 measured `top`; the boxes run `selective`. Same universe, same windows.\n");
  console.log(`  Universe: ${SYMS.join(",")} + ${INV.join(",")}   inverses list from ${dStart}\n`);
  const out = [];
  for (const [name, cfg, syms] of ROWS) { const r = run(cfg, syms); out.push({ name, r, cfg }); console.log(line(name, r)); }

  console.log("\n  DOWN DAYS ONLY (SPY close-to-close), hourly holdout h2 — the question that started this:");
  console.log(`  ${"mode".padEnd(30)}${"n".padStart(5)}${"WR".padStart(7)}${"mean/trade".padStart(13)}${"n up".padStart(7)}${"WR up".padStart(8)}${"mean up".padStart(11)}`);
  for (const o of out) {
    const tr = o.r.h.trades.filter((t) => t.d >= H_MID);
    const dn = tr.filter((t) => dirByDay.get(t.d) === "down"), up = tr.filter((t) => dirByDay.get(t.d) === "up");
    const m = (a) => (a.length ? ((a.reduce((s, t) => s + t.ret, 0) / a.length) * 100).toFixed(3) + "%" : "   -   ");
    const w = (a) => (a.length ? ((a.filter((t) => t.ret > 0).length / a.length) * 100).toFixed(0) + "%" : "  -  ");
    console.log(`  ${o.name.padEnd(30)}${String(dn.length).padStart(5)}${w(dn).padStart(7)}${m(dn).padStart(13)}${String(up.length).padStart(7)}${w(up).padStart(8)}${m(up).padStart(11)}`);
  }

  console.log("\n  INVERSE TRADES TAKEN (hourly, both halves) — how often each rule lets one through:");
  for (const o of out) {
    const inv = o.r.h.trades.filter((t) => INV.includes(t.sym));
    const m = inv.length ? ((inv.reduce((s, t) => s + t.ret, 0) / inv.length) * 100).toFixed(3) + "%" : "   -   ";
    const share = o.r.h.trades.length ? ((inv.length / o.r.h.trades.length) * 100).toFixed(1) + "%" : "-";
    console.log(`    ${o.name.padEnd(30)} ${String(inv.length).padStart(4)} inverse trades (${share} of all), ${m}/trade`);
  }
  console.log("    live, both boxes, 2026-08-18..25: 2 inverse entries of 23 (8.7%)");

  // STABILITY — a two-window pass on one split is not a finding if one year carries it.
  console.log("\n  PER-YEAR, daily holdout (return/DD ratio; the year each mode would have handed you):");
  const years = [...new Set(daily.SPY.filter((b) => b.d >= HOLD[0]).map((b) => b.d.slice(0, 4)))].sort();
  console.log(`  ${"mode".padEnd(30)}${years.map((y) => y.slice(2).padStart(8)).join("")}`);
  for (const o of out) {
    const cells = years.map((y) => {
      const s = score(o.r.d.curve, `${y}-01-01`, `${Number(y) + 1}-01-01`);
      return (s && s.tot != null ? (s.tot * 100).toFixed(0) + "%" : "-").padStart(8);
    });
    console.log(`  ${o.name.padEnd(30)}${cells.join("")}`);
  }
  console.log("\n  HOURLY HOLDOUT SPLIT IN HALF — does h2's answer hold in both of its own halves?");
  const h2mid = (() => { const bs = hourly.SPY.filter((b) => b.d >= H_MID); return bs[Math.floor(bs.length / 2)].d; })();
  console.log(`  ${"mode".padEnd(30)}${"h2a".padStart(16)}${"h2b".padStart(16)}`);
  for (const o of out) {
    const a = score(o.r.h.curve, H_MID, h2mid), b = score(o.r.h.curve, h2mid, END);
    console.log(`  ${o.name.padEnd(30)}${(pct(a.tot, 8) + " ÷" + ratio(a)).padStart(16)}${(pct(b.tot, 8) + " ÷" + ratio(b)).padStart(16)}`);
  }

  // the two-window verdict, stated the way every other lab in this repo states it
  const base = out[0], rat_ = (s) => rat(s);
  const fitOf = (o) => rat(o.r.s[0]) / rat(base.r.s[0]);
  const holdOf = (o) => rat(o.r.s[2]) / rat(base.r.s[2]);
  const cands = out.slice(1).sort((a, b) => fitOf(b) - fitOf(a));
  const win = cands[0];
  console.log(`\n  -> hourly fit winner: ${win.name} (h1 ÷${ratio(win.r.s[0])} vs longs-only ÷${ratio(base.r.s[0])})`);
  console.log(`  -> hourly holdout: ${holdOf(win) >= 1 ? "CONFIRMS" : "REJECTS"} (h2 ÷${ratio(win.r.s[2])} vs ÷${ratio(base.r.s[2])})`);
  const armed = out.find((o) => o.cfg.polarity === "selective");
  console.log(`  -> and the ARMED rule specifically: h1 ÷${ratio(armed.r.s[0])}, h2 ÷${ratio(armed.r.s[2])} vs longs-only ÷${ratio(base.r.s[0])} / ÷${ratio(base.r.s[2])}`);
