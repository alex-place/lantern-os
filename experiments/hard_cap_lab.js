/**
 * hard_cap_lab.js — the operator asked for "a hard 12% regardless of tilt"
 * (2026-08-25). This measures what that costs before it is armed.
 *
 * Today TRADER_SYMBOL_SIZE_MULT scales the per-position NOTIONAL CAP as well as the
 * risk target, so SOXL/SMH/QQQ (tilt 1.5) size to 18% of equity and 27% under the
 * stress multiplier, while SPY (0.83) and GLD/TLT (0.5) size below 12%. A hard cap
 * clamps the UP-weights at the cap and leaves the DOWN-weights alone, so it sits
 * between the armed stack and no tilt at all.
 *
 * Same harness as armed_baseline.js, same four surfaces, reading the live env.
 * Usage: node experiments/hard_cap_lab.js [--env C:/dev/lantern-os-stable/.env.local]
 */
"use strict";
  ...
  for (const t of timeline) {
    ...
    if (open.size < o.maxConc) {
      ...
      for (const c of cands) {
        ...
        // HARD CAP: tilt may size a position DOWN but never past the per-position
        // ceiling. This is what "a hard 12% regardless of tilt" means in the sizer —
        // SOXL/SMH/QQQ fall from 18% to 12%, the down-tilted names are untouched.
        if (o.hardCap) frac = Math.min(frac, o.sizeFrac);
...
(async () => {
  ...
  const nowSec = Math.floor(Date.now() / 1000), DAYS = 720;
  const hourly = {}, daily = {};
  for (const s of SYMS) { hourly[s] = annotate(await chart(s, "1h", nowSec - DAYS * 86400), true); daily[s] = annotate(await chart(s, "1d", Math.floor(Date.UTC(1999, 0, 1) / 1000)), false); }
  ...
  const runOne = (cfg) => {
    ...
    const o = {};
    for (const [k, a, z, src] of SURF2) {
      const x = src === "h" ? h : d;
      const s = score(x.curve, a, z), st = stats(x.trades, a, z);
      o[k] = { tot: +s.tot.toFixed(4), dd: +s.dd.toFixed(4), ratio: +(s.dd < 0 ? s.tot / -s.dd : 0).toFixed(2), n: st.n, wr: +st.wr.toFixed(3), b: +st.b.toFixed(2) };
    }
    return o;
  };
  const line = (name, o) => `  ${name.padEnd(30)}` + SURF2.map(([k]) => `${k} ${pct(o[k].tot, 8)} ÷${String(o[k].ratio).padStart(7)} wr ${(o[k].wr * 100).toFixed(0)}%`).join(" | ");

  console.log(`HARD PER-POSITION CAP — may the symbol tilt size a position ABOVE the cap, or only below it?
`);
  console.log(`  env: ${envPath}`);
  console.log(`  cap ${(ARMED.sizeFrac * 100).toFixed(0)}%  tilt ${JSON.stringify(ARMED.weights)}
`);

  const asArmed = runOne({ ...ARMED, hardCap: false });
  const hardCap = runOne({ ...ARMED, hardCap: true });
  const flat = runOne({ ...ARMED, hardCap: false, weights: null });
  console.log(line("as armed (tilt scales the cap)", asArmed));
  console.log(line("HARD CAP (tilt only sizes down)", hardCap));
  console.log(line("no tilt at all (flat)", flat));

  const cost = (a, b, k) => {
    const ra = a[k].ratio, rb = b[k].ratio;
    return rb === 0 ? "n/a" : `${(((ra / rb) - 1) * 100).toFixed(0)}%`;
  };
  console.log(`
  hard cap vs as armed, return/DD ratio: ` + SURF2.map(([k]) => `${k} ${cost(hardCap, asArmed, k)}`).join("  "));
  console.log(`  no tilt   vs as armed, return/DD ratio: ` + SURF2.map(([k]) => `${k} ${cost(flat, asArmed, k)}`).join("  "));
  console.log(`
  (a hard cap keeps the tilt's DOWN-weights — SPY 0.83, GLD/TLT 0.5 — and removes only its`);
  console.log(`   UP-weights, so it should land between "as armed" and "no tilt at all".)`);
})().catch((e) => { console.error("hard_cap_lab failed:", e.message); process.exit(1); });
