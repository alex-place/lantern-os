/**
 * tilt_walkforward_lab.js — static vs walk-forward symbol tilt (round 8, 2026-08-23).
 * The armed tilt (#3434) was chosen once on the 2000-14 fit window and now drives
 * both sizing and slot priority (#3438). Does re-estimating the weights every
 * year from the trailing N years of per-symbol expectancy beat the static set
 * out of sample? Walk-forward on the daily surface 2015-2026 (weights for year Y
 * from years Y-N..Y-1, single-symbol sims, clamp(edge/median, 0.5, 1.5)); the
 * hourly halves use weights from the 3y ending at each half's start.
 * Usage: node experiments/tilt_walkforward_lab.js [--days 720]
 */
"use strict";
...
(async () => {
  ...
  const STATIC = { SOXL: 1.5, SMH: 1.5, QQQ: 1.5, IWM: 1.02, XLK: 1.0, SPY: 0.83, DIA: 0.71, GLD: 0.5, TLT: 0.5 };
  const BASE = { ...MONDAY, slotOrder: "expectancy" };
  const yrs = (d) => Number(d.slice(0, 4));
  // per-symbol expectancy over [from,to) on daily bars (single-symbol sims, no VIX sizing)
  const edgeOf = (sym, from, to) => { const r = simulate(daily, [sym], { ...MONDAY, vixUp: null, weights: null }, false, vix, from, to); return r.trades.length >= 20 ? r.trades.reduce((a, x) => a + x.ret, 0) / r.trades.length : null; };
  const weightsFrom = (from, to, lo = 0.5, hi = 1.5) => {
    const e = {}; for (const s of SYMS) e[s] = edgeOf(s, from, to);
    const vals = Object.values(e).filter((x) => x != null).sort((a, b) => a - b);
    const med = vals.length ? vals[Math.floor(vals.length / 2)] : 0;
    const w = {}; for (const s of SYMS) w[s] = e[s] == null || !(med > 0) ? 1 : Math.min(hi, Math.max(lo, e[s] / med));
    return w;
  };
  const chain = (N, lo, hi) => {   // walk-forward over the daily holdout: one sim per year with that year's trailing weights, equity chained
    let eq = 1, peak = 1, dd = 0; const segs = [];
    for (let y = 2015; y <= 2026; y++) {
      const w = weightsFrom(`${y - N}-01-01`, `${y}-01-01`, lo, hi);
      const r = simulate(daily, SYMS, { ...BASE, weights: w }, false, vix, `${y}-01-01`, `${y + 1}-01-01`);
      const seg = r.curve.filter((p) => p.d >= `${y}-01-01` && p.d < `${y + 1}-01-01`);
      if (seg.length < 10) continue;
      const e0 = seg[0].equity;
      for (const p of seg) { const v = eq * (p.equity / e0); peak = Math.max(peak, v); dd = Math.min(dd, v / peak - 1); }
      eq *= seg[seg.length - 1].equity / e0;
      segs.push({ y, w: Object.fromEntries(SYMS.map((s) => [s, +w[s].toFixed(2)])) });
    }
    return { tot: eq - 1, dd, segs };
  };
  const cell = (s) => `${pct(s.tot, 7)} ÷${(s.dd < 0 ? s.tot / -s.dd : 0).toFixed(2).padStart(6)} dd ${pct(s.dd, 5)}`;
  console.log("DAILY HOLDOUT 2015-2026 — static vs walk-forward tilt (sizing + slot priority both follow the weights)");
  for (const [name, w] of [["flat", null], ["static fit tilt (armed)", STATIC]]) {
    const r = simulate(daily, SYMS, { ...BASE, weights: w }, false, vix, "2015-01-01", "2099-01-01");
    const s = score(r.curve, "2015-01-01", "2099-01-01");
    console.log(`  ${name.padEnd(40)} ${cell(s)}`);
  }
  for (const [N, lo, hi] of [[3, 0.5, 1.5], [5, 0.5, 1.5], [2, 0.5, 1.5], [3, 0.25, 2.0], [3, 0.5, 2.0]]) {
    const s = chain(N, lo, hi);
    console.log(`  ${`walk-forward trailing ${N}y, clamp ${lo}-${hi}`.padEnd(40)} ${cell(s)}`);
    if (N === 3 && lo === 0.5 && hi === 1.5) console.log("    weights by year: " + s.segs.map((x) => `${x.y}: SOXL ${x.w.SOXL} SMH ${x.w.SMH} QQQ ${x.w.QQQ} SPY ${x.w.SPY} TLT ${x.w.TLT}`).join(" | "));
  }
  console.log(`\nHOURLY 2y halves — weights from the trailing 3y of daily bars ending at each half's start`);
  const w1 = weightsFrom(`${yrs(H_START) - 3}-01-01`, H_START), w2 = weightsFrom(`${yrs(H_MID) - 3}-01-01`, H_MID);
  console.log("  h1 weights: " + SYMS.map((s) => `${s} ${w1[s].toFixed(2)}`).join(" ") + "\n  h2 weights: " + SYMS.map((s) => `${s} ${w2[s].toFixed(2)}`).join(" "));
  for (const [name, wa, wb] of [["flat", null, null], ["static fit tilt (armed)", STATIC, STATIC], ["walk-forward 3y", w1, w2]]) {
    const ra = simulate(hourly, SYMS, { ...BASE, weights: wa }, true, vix), rb = simulate(hourly, SYMS, { ...BASE, weights: wb }, true, vix);
    const s1 = score(ra.curve, H_START, H_MID), s2 = score(rb.curve, H_MID, H_END);
    console.log(`  ${name.padEnd(26)} h1 ${cell(s1)} | h2 ${cell(s2)}`);
  }
