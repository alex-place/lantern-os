/**
 * step_floor_lab.js — the stepped profit floor (operator, 2026-08-24 after the
 * first live day): "if it's at 1%+ the exit should be 1%, if 2%+ it should be
 * 2%, if 3%+ it should be 3% — currently it doesn't trail high enough and
 * doesn't adapt to different position types."
 *
 * Today SOXL reached +4.13% and its floor stayed at entry+1% (be_ratchet fired
 * once at 11:31 and never moved again); the book captured 44% of the MFE it
 * reached. The change: as the PEAK gain crosses each step, the stop ratchets to
 * that many percent above entry and never lowers — a small position locks 1%,
 * a SOXL-style runner locks 4%. Swept against the flat +1% floor on the four
 * surfaces, with and without the existing trail.
 * Usage: node experiments/step_floor_lab.js
 */
"use strict";
...
(async () => {
  ...
  const cell = (r, i) => `b ${r.st[i].b.toFixed(2)} wr ${(r.st[i].wr * 100).toFixed(0)}% aw ${(r.st[i].aw * 100).toFixed(2)}% ${pct(r.s[i].tot, 7)} ÷${ratio(r.s[i])}`;
  ...
  const sweep = (title, variants, note) => {
    ...
    console.log(`  → fit winner: ${w.name}${/armed/.test(w.name) ? " — the armed setting" : ` — holdout ${hOk ? "CONFIRMS" : "REJECTS"} (h2 ÷${ratio(w.r.s[2])} vs ${ratio(base.s[2])}, d ÷${ratio(w.r.s[3])} vs ${ratio(base.s[3])})`}`);
    const better = rows.filter((x) => x !== rows[0] && rat(x.r.s[2]) >= rat(base.s[2]) && rat(x.r.s[3]) >= rat(base.s[3]));
    console.log(`  → beats armed on BOTH holdouts: ${better.length ? better.map((x) => x.name).join("; ") : "none"}`);
    if (note) console.log("  " + note);
  };
  console.log("STEPPED PROFIT FLOOR — today SOXL reached +4.1% and its floor stayed at entry+1%.");
  console.log("Operator rule: each whole percent of PEAK gain locks that percent in (1%+ -> lock 1%, 2%+ -> 2%, 3%+ -> 3% ...).");
  console.log(line("armed (flat +1% floor)", base));
  sweep("A. STEP SIZE — lock each whole step of peak gain (never lowers; the 3% stop and the bounce exit still apply)", [
    ["armed (flat +1%)", ARMED],
    ["step 1% (the operator rule)", { ...ARMED, stepFloor: 1 }],
    ["step 0.5%", { ...ARMED, stepFloor: 0.5 }],
    ["step 2%", { ...ARMED, stepFloor: 2 }],
    ["step 1%, give back 0.5%", { ...ARMED, stepFloor: 1, stepGive: 0.5 }],
    ["step 1% only (no flat floor)", { ...ARMED, be: null, stepFloor: 1 }],
  ]);
  sweep("B. STEP + the existing trail — does the step floor replace the trail, or complement it?", [
    ["armed", ARMED],
    ["step 1%", { ...ARMED, stepFloor: 1 }],
    ["step 1%, no trail", { ...ARMED, stepFloor: 1, trail: null }],
    ["no step, no trail (floor only)", { ...ARMED, trail: null }],
    ["step 0.5%, no trail", { ...ARMED, stepFloor: 0.5, trail: null }],
  ]);
  console.log("\nC. WHAT IT DOES TO THE AVERAGE WIN (hourly h2, the surface closest to live)");
  for (const [name, cfg] of [["armed (flat +1%)", ARMED], ["step 1%", { ...ARMED, stepFloor: 1 }], ["step 0.5%", { ...ARMED, stepFloor: 0.5 }]]) {
    const r = run(cfg); anatomy(r.h.trades, H_MID, END, name);
  }
