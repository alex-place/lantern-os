/**
 * tail_lab.js — the armed stack's WORST months and what drives them (2026-08-23).
 * The operator's standing objection is the drawdown. The stress multiplier
 * (x1.5 at VIX >= 20) and the tilt (1.5x SOXL/SMH/QQQ) both concentrate size
 * exactly when markets break, so the tail is where the armed stack must be read.
 * Daily 26y + hourly 2y: month-by-month returns of the armed stack; the ten worst
 * and ten best months; the same months WITHOUT the stress multiplier, at half
 * size (half-Kelly 7.4%), and without the tilt; the worst-week drawdown path in
 * 2008-Q4, 2020-03, 2024-08 and 2025-04. Usage: node experiments/tail_lab.js
 */
"use strict";
...
function monthly(curve, from, to) {
  const seg = curve.filter((p) => p.d >= from && p.d < to);
  const byM = new Map();
  for (const p of seg) { const m = p.d.slice(0, 7); if (!byM.has(m)) byM.set(m, { first: p.equity, last: p.equity, min: p.equity, peak: p.equity, dd: 0 }); const x = byM.get(m); x.last = p.equity; x.peak = Math.max(x.peak, p.equity); x.dd = Math.min(x.dd, p.equity / x.peak - 1); }
  const ms = [...byM.keys()], out = [];
  for (let i = 0; i < ms.length; i++) { const start = i === 0 ? byM.get(ms[i]).first : byM.get(ms[i - 1]).last; out.push({ m: ms[i], ret: byM.get(ms[i]).last / start - 1, intra: byM.get(ms[i]).dd }); }
  return out;
}
function ddPath(curve, from, to) {
  const seg = curve.filter((p) => p.d >= from && p.d < to);
  let peak = -Infinity, worst = 0, worstD = null, peakD = null, recD = null;
  for (const p of seg) { if (p.equity > peak) { peak = p.equity; peakD = p.d; } const dd = p.equity / peak - 1; if (dd < worst) { worst = dd; worstD = p.d; } }
  // recovery: first date after worstD where equity >= the peak before it
  if (worstD) { const pk = Math.max(...seg.filter((p) => p.d <= worstD).map((p) => p.equity)); const r = seg.find((p) => p.d > worstD && p.equity >= pk); recD = r ? r.d : "not yet"; }
  return { worst, worstD, recD, ret: seg.length ? seg[seg.length - 1].equity / seg[0].equity - 1 : 0 };
}
(async () => {
  ...
  const ARMED = { ...MONDAY, weights: WEIGHTS, slotOrder: "expectancy" };
  const VARIANTS = [
    ["armed (12%, tilt, stress x1.5)", ARMED],
    ["no stress multiplier", { ...ARMED, vixUp: null }],
    ["half size (7.4% ≈ half-Kelly)", { ...ARMED, sizeFrac: 0.074 }],
    ["no tilt (flat 12%)", { ...ARMED, weights: null, slotOrder: "depth" }],
    ["cap 3 positions", { ...ARMED, maxConc: 3 }],
  ];
  const D0 = "2000-01-01", END = "2099-01-01";
  const runs = VARIANTS.map(([name, cfg]) => ({ name, d: simulate(daily, SYMS, cfg, false, vix), h: simulate(hourly, SYMS, cfg, true, vix) }));
  const base = runs[0];
  const mB = monthly(base.d.curve, D0, END);
  console.log("DAILY 26y — the armed stack's month distribution");
  const sorted = mB.slice().sort((a, b) => a.ret - b.ret);
  const q = (p) => sorted[Math.floor(p * (sorted.length - 1))].ret;
  console.log(`  months ${mB.length}  red ${mB.filter((x) => x.ret < 0).length}  median ${pct(q(0.5))}  p10 ${pct(q(0.1))}  p5 ${pct(q(0.05))}  worst ${pct(sorted[0].ret)}  best ${pct(sorted[sorted.length - 1].ret)}`);
  console.log("  worst 10 months — armed vs the variants (same month):");
  const mOf = (r) => { const m = new Map(monthly(r.d.curve, D0, END).map((x) => [x.m, x])); return m; };
  const maps = runs.map(mOf);
  console.log("  " + "month".padEnd(9) + VARIANTS.map(([n]) => n.padStart(30)).join(""));
  for (const x of sorted.slice(0, 10)) console.log("  " + x.m.padEnd(9) + maps.map((m) => pct((m.get(x.m) || {}).ret || 0, 7).padStart(30)).join(""));
  console.log("  best 10 months:");
  for (const x of sorted.slice(-10).reverse()) console.log("  " + x.m.padEnd(9) + maps.map((m) => pct((m.get(x.m) || {}).ret || 0, 7).padStart(30)).join(""));
  console.log("\n  totals / max drawdown / worst month / red months per variant (daily 26y, then hourly 2y):");
  for (const r of runs) { const s = score(r.d.curve, D0, END), sh = score(r.h.curve, hourly.SPY[0].d, END); console.log(`  ${r.name.padEnd(32)} 26y ${pct(s.tot, 9)} ÷${ratio(s)} dd ${pct(s.dd, 6)} worst-mo ${pct(s.worst, 6)} red ${s.neg}/${s.months}   | 2y ${pct(sh.tot, 6)} ÷${ratio(sh)} dd ${pct(sh.dd, 6)} worst-mo ${pct(sh.worst, 6)} red ${sh.neg}/${sh.months}`); }
  console.log("\nCRISIS WINDOWS — armed stack drawdown path (daily): worst point, date, recovery");
  for (const [label, a, z] of [["2008-Q4", "2008-09-01", "2009-06-30"], ["2011-Aug", "2011-07-15", "2011-12-31"], ["2018-Q4", "2018-10-01", "2019-03-31"], ["2020-Mar", "2020-02-15", "2020-06-30"], ["2022", "2022-01-01", "2022-12-31"], ["2024-Aug", "2024-07-15", "2024-09-30"], ["2025-Apr", "2025-03-15", "2025-06-30"]]) {
    const cells = runs.slice(0, 3).map((r) => { const p = ddPath(r.d.curve, a, z); return `${r.name.split(" (")[0].padEnd(20)} ret ${pct(p.ret, 6)} dd ${pct(p.worst, 6)} @ ${p.worstD || "-"} rec ${p.recD || "-"}`; });
    console.log(`  ${label.padEnd(9)} ${cells.join("  |  ")}`);
  }
  console.log("\nHOURLY 2y — worst weeks of the armed stack (weekly return), with the variants");
  const weekly = (curve) => { const byW = new Map(); for (const p of curve) { const d = new Date(p.d + "T12:00:00Z"); const wk = new Date(d); wk.setUTCDate(d.getUTCDate() - ((d.getUTCDay() + 6) % 7)); const k = wk.toISOString().slice(0, 10); if (!byW.has(k)) byW.set(k, { first: p.equity, last: p.equity }); byW.get(k).last = p.equity; } const ks = [...byW.keys()]; return ks.map((k, i) => ({ w: k, ret: byW.get(k).last / (i ? byW.get(ks[i - 1]).last : byW.get(k).first) - 1 })); };
  const wB = weekly(base.h.curve).sort((a, b) => a.ret - b.ret).slice(0, 6);
  const wMaps = runs.map((r) => new Map(weekly(r.h.curve).map((x) => [x.w, x.ret])));
  console.log("  " + "week of".padEnd(12) + VARIANTS.map(([n]) => n.padStart(30)).join(""));
  for (const x of wB) console.log("  " + x.w.padEnd(12) + wMaps.map((m) => pct(m.get(x.w) || 0, 7).padStart(30)).join(""));
})().catch((e) => { console.error("tail lab failed:", e.message); process.exit(1); });
