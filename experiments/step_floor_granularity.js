...
(async () => {
  ...
  const IV = [["1h", 60], ["30m", 30], ["15m", 15], ["5m", 5]];
  const bars = {};
  ...
  const from = IV.map(([iv]) => bars[iv].SPY[0].d).sort().slice(-1)[0], to = "2099-01-01";
  ...
  console.log(`GRANULARITY CHECK — same window ${from}..today on four bar sizes.`);
  console.log("If the step floor is real the gain survives as bars get finer; if it is an intrabar artifact it collapses at 5m (the round-6 ratchet-trail failure).\n");
  const cells = (cfg, iv) => { const r = simulate(bars[iv], SYMS, cfg, true, vix, from, to); const s = score(r.curve, from, to), st = stats(r.trades, from, to); return `n ${String(st.n).padStart(3)} wr ${(st.wr*100).toFixed(0)}% b ${st.b.toFixed(2)} ${pct(s?s.tot:0,6)} dd ${pct(s?s.dd:0,5)}`; };
  for (const [name, cfg] of [["armed (flat +1% floor)", ARMED], ["step 1%", { ...ARMED, stepFloor: 1 }], ["step 0.5%", { ...ARMED, stepFloor: 0.5 }], ["step 2%", { ...ARMED, stepFloor: 2 }]]) {
    console.log("  " + name.padEnd(24) + IV.map(([iv]) => iv.padStart(4) + ": " + cells(cfg, iv)).join(" | "));
  }
  console.log("\n  read: compare each step row against the armed row AT THE SAME bar size.");
