/**
 * armed_baseline.js — the armed stack on the four validation surfaces, as one
 * line (round 8, 2026-08-23). Reads the live env (a .env.local path, default
 * the stable box) so the backtest is of the configuration that is actually
 * running: IBS max / morning / exit level / floor / stop / tilt / slot order /
 * stress. The periodic check re-runs this whenever the boxes' code or env
 * changes, so every change is scored on the same bar it was validated on.
 * Usage: node experiments/armed_baseline.js [--env C:/dev/lantern-os-stable/.env.local] [--json]
 */
"use strict";
...
})().catch((e) => { console.error("armed_baseline failed:", e.message); process.exit(1); });
