...
| `research/epistemic_controller/run_world_s.py` | registered here — world S, the SELF experiment: can the scientist diagnose a defect in its own experiment-selection policy from its own mistake ledger? **KILLED** — the strict evidence bar is specific (0% false firing on both controls) but almost never fires and buys 0.8%; loosening it doubles firing and fires on the controls (12%/20%). Imports `research/epistemic_controller/self_model.py` | `python research/epistemic_controller/run_world_s.py 100` |
