...
| `research/epistemic_controller/run_world_h.py` | registered here — world H, the kill test for retraction-by-remedy-failure: two explanations fit the data; shipped controller 66% truth (INCONCLUSIVE), hold-the-expansion variant 86% (PASS). Imports `research/epistemic_controller/environments/two_explanations.py` | `python research/epistemic_controller/run_world_h.py 200` |
| `research/epistemic_controller/run_discovery_benchmark.py` | registered here — the Adam-style benchmark: validated regularities per experiment over a SEQUENCE of hidden rules. Result 2026-07-27: naive "scientific memory" across episodes is KILLED (0.107 → 0.056 per experiment, 80 false discoveries); memory of a NAME is not memory of a regularity. Imports `research/epistemic_controller/run_world_h.py` | `python research/epistemic_controller/run_discovery_benchmark.py 60` |
